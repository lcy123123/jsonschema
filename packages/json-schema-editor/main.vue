<template>
  <div class="json-schema-editor">
    <a-row class="row" :gutter="4">
      <a-col :span="9" class="ant-col-name">
        <div :style="{marginLeft:`${5*deep}px`}" class="ant-col-name-c">
          <a-button
      ...
      <a-col :span="2.5">
        ...
        >
          <a-select-option :key="t" v-for="t in TYPE_NAME">{{t}}</a-select-option>
        </a-select>
      </a-col>
       <a-col :span="5" >
        <a-input
      ...
      <!-- <a-col>
        <a-input
          :value="pickValue.remark"
          class="ant-col-title"
          :placeholder="local['remark']"
          @blur="onInputRemark"
        />
      </a-col> -->
      <a-col :span="4" class="ant-col-setting">
        <!-- 新加敏感信息按钮 -->
        <!-- v-if="actualShowSensitive" -->
        <a-tooltip>
          <span slot="title">敏感信息</span>
          <a-button type="link" icon="file-protect" class="setting-icon" @click="onSensitive" />
        </a-tooltip>
        <a-tooltip>
    ...
    > -->
      ...
      <a-form v-model="advancedValue" class="ant-advanced-search-form">
        <a-row :gutter="20">
          <a-col :span="24" v-for="(item,key) in advancedValue" :key="key">
            <a-form-item style="margin-bottom:0">
              ...
              >
                ...
                <a-select-option :key="t" v-for="t in advancedAttr[key].enums">{{t}}</a-select-option>
              </a-select>
              <!-- <a-input v-model="advancedValue[key]" v-else style="width:100%" :placeholder="key" /> -->
              <a-input v-model="advancedValue[key]" :type="key=='description'?'textarea':'text'" v-else style="width:100%" :placeholder="key" />
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
...
import {
  isNull,
  renamePropertyAndKeepKeyPrecedence,
  arrayDifference
} from "./util";
...
export default {
  ...
  provide() {
    return {
      // 提供一个获取敏感按钮状态的函数，这样子组件可以继承这个状态
      getSensitiveEnabled: () => this.showSensitive
    };
  },
  inject: {
     getSensitiveEnabled: {
    ...
    // 新增：接收全局敏感点击回调
    handleSensitiveGlobal: {
      default: null
    }
  },
  computed: {
     actualShowSensitive() {
      return this.getSensitiveEnabled ? this.getSensitiveEnabled() : false;
    },
    pickValue() {
  ...
  methods: {
    ...
    onInputRemark(e) {
      console.log(e);
      this.$set(this.pickValue, "remark", e.target.value);
    },
    onChangeType() {
    ...
    emitSensitiveClick(data) {
    // 直接触发当前组件实例的事件
    this.$emit('sensitive-click', data);
  },
    // 敏感信息方法
    // onSensitive() {
    //   // 向外暴露事件，传递当前节点的关键数据
    //   this.$emit("sensitive-click", {
    //     key: this.pickKey, // 当前字段名
    //     schema: this.pickValue, // 当前节点的 schema 对象
    //     parent: this.parent, // 父节点对象（可用于修改 required 等）
    //     updateSchema: updates => {
    //       // 提供便捷的更新方法
    //       Object.keys(updates).forEach(k => {
    //         this.$set(this.pickValue, k, updates[k]);
    //       });
    //     }
    //   });
    // },
      ...
      // 2. 优先调用全局注入的方法（递归所有层级都能生效）
      if (this.handleSensitiveGlobal) {
        this.handleSensitiveGlobal(params);
      }
      // 保留原emit，兼容外层根组件事件（可选）
      this.$emit("sensitive-click", params);
    },
    onSetting() {
